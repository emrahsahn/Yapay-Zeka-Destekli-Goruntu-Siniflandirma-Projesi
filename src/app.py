...
Supported Animals (10 classes):
    🐕 Köpek (Dog)       🐴 At (Horse)        🐘 Fil (Elephant)
    🦋 Kelebek (Butterfly) 🐔 Tavuk (Chicken)   🐱 Kedi (Cat)
    🐄 İnek (Cow)        🐑 Koyun (Sheep)     🕷️ Örümcek (Spider)
    🐿️ Sincap (Squirrel)
"""
...
def predict(image):
    ...
    Example:
        ...
        >>> result = predict(img)
        >>> print(result)
        {"Köpek": 0.85, "Kedi": 0.10, "At": 0.03, ...}
    """
    # Error Handling: Model Not Loaded
    # --------------------------------
    if model is None:
        return {"⚠ Hata: Model yüklenmedi": 1.0}
...
with gr.Blocks(title="Yapay Zeka Destekli Görüntü Sınıflandırıcı") as demo:
    ...
    gr.Markdown(
        ...
        Bu uygulama, yüklediğiniz hayvan fotoğraflarını yapay zeka ile analiz eder 
        ve hangi hayvana ait olduğunu yüksek doğrulukla tahmin eder.
        
        **Tanımlanabilen Hayvanlar (10 Sınıf):**  
        🐕 Köpek • 🐴 At • 🐘 Fil • 🦋 Kelebek • 🐔 Tavuk  
        🐱 Kedi • 🐄 İnek • 🐑 Koyun • 🕷️ Örümcek • 🐿️ Sincap
        
        ---
        **Kullanım:** Bir hayvan fotoğrafı yükleyin ve "Analiz Et" butonuna tıklayın.
        """
    )
    ...
    with gr.Row():
        ...
        with gr.Column():
            ...
            predict_btn = gr.Button("🔍 Analiz Et", variant="primary", size="lg")
